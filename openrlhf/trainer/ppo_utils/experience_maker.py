...
class RemoteExperienceMaker(NaiveExperienceMaker):
    ...
    def process_reformat_and_tokenize(self, input_ids, attention_mask, action_mask):
        ...
        if self.strategy.get_rank() == 0:
            for resp in responses:
                assert "<|assistant|>" not in resp, f"responses: {resp}"
                # print(resp)
        ...
        for i in range(len(responses)):
            ...
            responses[i] = reformat_response_into_steps(responses[i], use_nk=False, flag=process_flag)
    ...
    def apply_process_rewards(self, actions, rewards, batch_size, is_ppo=False, normalize_multi_traces=False, original_num_actions=-1):
        ...
        for reward_item, process_flag_item, action_item in zip(rewards, process_flag, actions):
            ...
            rewards_valid.append(reward_item)
            ...
            process_flag_item = (reward_item != 0).float()
            process_tokens_valid.append(process_flag_item)
        ...
        # assert rewards.shape == action_tokens.shape, f"rewards: {rewards.shape}, action_tokens: {action_tokens.shape}"
        ...
        if normalize_multi_traces:
            ...
            _process_flag = process_flag.view(batch_size, -1)
            ...
            reward_std = (((rewards - reward_mean) ** 2) * _process_flag).sum(1) / (_process_flag.sum(1) + 1e-7)
    ...
    def sample_responses_prm(self, prompts: List[str], num_trace_per_sample: int = 1, **generate_kwargs):
        ...
        actions_for_rm = zero_pad_batch(_actions_for_rm, side="right")

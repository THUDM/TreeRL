...
from openrlhf.datasets.reward_dataset import reformat_response_into_steps, revert_special_tokens, get_process_flag_tokens

logger = init_logger(__name__)


# normalize reward
def normalize_reward_from_multi_traces(reward, batch_size, num_trace_per_sample):
...
class RemoteExperienceMaker(NaiveExperienceMaker):
    ...
    def retokenize(self, input_ids, source_tokenizer, target_tokenzier):
        ...
        batch_input_ids = [target_tokenzier.encode(e) for e in texts]
        # input_ids = batch_inputs["input_ids"]
        # attention_mask = batch_inputs["attention_mask"]
        batch = target_tokenzier.batch_encode_plus(batch_input_ids, return_tensors="pt", is_split_into_words=True, padding=True)
    ...
    def process_reformat_and_tokenize(self, input_ids, attention_mask, action_mask):
        ...
        action_attention_mask = attention_mask[:, input_len:]
        responses = self.tokenizer.decode(actions[0], skip_special_tokens=True)
        # for resp in responses:
        for i in range(len(responses)):
            responses[i] = reformat_response_into_steps(responses[i])
        responses = self.tokenizer(responses, add_special_tokens=False)
        max_length = max([len(x) for x in responses["input_ids"]])
        reencode_input_ids = [x + [0] * max_length - len(x) for x in responses["input_ids"]]
    ...
    def revert_prm_tokens(self, input_ids):
        input_ids = revert_special_tokens(input_ids)
        return input_ids
    
    def apply_process_rewards(self, input_ids, action_mask, rewards, batch_size, is_ppo=False):
        input_len = action_mask[0].argmax().item()
        ...
        for token in self.process_flag_tokens:
            process_flag += (action_tokens == token).long()
        process_flag = (process_flag > 0).long()

        action_len = action_tokens.shape[-1]
        
        rewards = rewards * process_flag
        rewards = rewards.view(batch_size, -1)
        reward_mean = (rewards.sum(dim=1) / process_flag.sum(1)).view(-1, 1)
        ...
        if is_ppo:
            return normalized_rewards * process_flag
        ...
        for _ in range(num_trace_per_sample):
            ...
            for i in range(batch_size_multiplier):
                ...
                for rm in self.reward_model:
                    ...
                    r_refs.append(rm.forward.remote(micro_sequences_cpu_rm, micro_attention_mask_cpu_rm, False, True))
    ...
    def sample_responses(self, prompts: List[str], num_trace_per_sample: int = 1, **generate_kwargs):
        ...
        for _ in range(num_trace_per_sample):
            ...
            for i in range(batch_size_multiplier):
                ...
                for rm in self.reward_model:
                    ...
                    r_refs.append(rm.forward.remote(micro_sequences_cpu_rm, micro_attention_mask_cpu_rm))
        ...
        # print(f"action_mask: {action_mask.shape}, attention_mask: {attention_mask.shape}, sequences: {sequences.shape}, action_log_probs: {action_log_probs.shape}, base_action_log_probs: {base_action_log_probs.shape}, r: {r.shape}")

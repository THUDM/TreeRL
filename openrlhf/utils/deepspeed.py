...
class DeepspeedStrategy(ABC):
    ...
    # def setup_distributed(self, timeout=timedelta(minutes=60)) -> None:
        ...
        if self.args.use_mpi_init:
            self.args.local_rank = int(os.environ['OMPI_COMM_WORLD_LOCAL_RANK'])
            self.args.world_size = int(os.environ['OMPI_COMM_WORLD_SIZE'])
            self.args.rank = int(os.environ['OMPI_COMM_WORLD_RANK'])
        else:
                    ...
                    if param.requires_grad:
                        if self.stage != 3:
                            ...
                            with deepspeed.zero.GatheredParameters(params_to_fetch, enabled=len(params_to_fetch) > 0):
                                # TODO: bug
                                # print(f"param: {param.data.device}, param_ema: {param_ema.data.device}")
                                device = param_ema.data.device
                                data = param.data.to(device)
                                # param_ema.data.copy_((1 - beta) * data + beta * param_ema.data)
                                param_ema.data.copy_(torch.lerp(data, param_ema.data, beta))

                            # with deepspeed.zero.GatheredParameters([param], enabled=True):
                                # with deepspeed.zero.GatheredParameters([param_ema], enabled=True):
                                    # data = param.data.to(device)
                                    # param_ema.data.copy_((1 - beta) * data + beta * param_ema.data)
                                
    def copy_model_weight(self, target_model, source_model, device="cpu"):
        with torch.no_grad():
